...
namespace seneca
{
	...
	int ProcessData::operator()(const std::string& filename, double& avgValue, double& compValue){
		...
		for(int i =0; i< num_threads;i++){
			threads.push_back(std::thread(std::bind(computeAvgFactor,data+p_indices[i],p_indices[i+1]-p_indices[i],p_indices[i+1]-p_indices[i],std::ref(averages[i]))));
		}
		for(auto & t: threads){
			t.join();
		}
		...
		avgValue/= num_threads;
		//const int* arr is a pointer. it points to const int type。这个指针指向数组的第一个元素，因此可以用于遍历数组中的所有元素。
		threads.clear();
		for(int i=0;i<num_threads;i++){
			threads.push_back(std::thread(std::bind(computeVarFactor,data+p_indices[i],p_indices[i+1]-p_indices[i], total_items, avgValue,std::ref(variances[i]))));
		}
		for(auto & t: threads){
			t.join();
		}
