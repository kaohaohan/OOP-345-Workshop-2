...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		size_t count =0;
		do {
			//inputFile is the one line
			std::getline(inputFile, book);
			if (inputFile) {
				if (book[0] != '#') {
	...
	auto fixPrice=[=](seneca::Book& book){
		...
		if(book.country()=="UK" && 1990<=book.year() &&book.year() <=1999){
	...
	if (argc > 2) {
		...
		do {
			//inputFile is the one line
			std::getline(inputFile, movie);
			if (inputFile) {
				if (movie[0] != '#') {
				 movies[count]=seneca::Movie(movie);
					count++;
				}
		}
		} while (inputFile && count < 5);
	}
