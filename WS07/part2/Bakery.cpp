    ...
    std::ostream &operator<<(std::ostream &out, const BakedGood &b) {
     ...
     void Bakery::showGoods(std::ostream& os) const{
            ...
            //use for each 
             ...
             //use lambda 
             int stockSum = std::accumulate(m_goods.begin(), m_goods.end(), 0,
                [](int total, BakedGood current) {
                    return total + current.m_stock;
                });
              double priceSum = std::accumulate(m_goods.begin(), m_goods.end(), 0.0,
                [](double total, BakedGood current) {
     ...
     void  Bakery::sortBakery(const std::string field){
      ...
      std::vector<BakedGood> Bakery::combine(const Bakery &b){
            //b.m_goods inserts
            //copy m_goods 
            std::vector<BakedGood> copy(m_goods);
            std::move(b.m_goods.begin(), b.m_goods.end(), std::back_inserter(copy));
     ...
     bool Bakery::inStock(const std::string description, const BakedType type) const{
     ...
     std::list<BakedGood> Bakery::outOfStock(const BakedType type) const{
        ...
        std::copy_if(m_goods.begin(), m_goods.end(), std::back_inserter(result), [&type](BakedGood good) {
